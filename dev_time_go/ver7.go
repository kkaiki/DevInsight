...
func getSortedDiscordData() []DiscordWorkTime {
    ...
    if err != nil {
        log.Printf("Discord IDの取得エラー: %v", err)
        return nil
    }

    if len(discordIDs) == 0 {
        log.Printf("対象期間内のデータが見つかりません")
        return nil
    }
    ...
    for _, discordID := range discordIDs {
        ...
        if err != nil {
            log.Printf("時間データの取得エラー (Discord ID: %s): %v", discordID, err)
            continue
        }
        ...
        if len(times) > 0 {
            ...
            log.Printf("ユーザー %s の作業時間: %v", discordID, totalWorkTime)
        }
    }

    if len(data) == 0 {
        log.Printf("集計可能なデータが見つかりません")
        return nil
    }
